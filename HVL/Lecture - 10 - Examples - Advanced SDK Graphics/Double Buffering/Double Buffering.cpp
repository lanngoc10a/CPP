// Double Buffering.cpp : Defines the entry point for the application.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        PAINTSTRUCT ps;
        // notice that I used phdc as the variable name for our physical device contexts
        // This is use to free up hdc so I can use it as the "default" name in the calls.
        ...
        HBRUSH circle = CreateSolidBrush(RGB(0, 0, 180));
        SelectObject(hdc, circle);

        double origin = (360 - rot) * M_PI / 180;
        int x = (int)(cos(origin) * screen.right / 4);
        int y = (int)(sin(origin) * screen.bottom / 4);

        Ellipse(hdc, x - 25 + screen.right / 2, y - 25 + screen.bottom / 2, x + 25 + screen.right / 2, y + 25 + screen.bottom / 2);
